...
void setup(char *data_file)
{
	...
	combi_malloc<Lattice>(&lattice_host, &lattice_device, sizeof(Lattice)); //combi_malloc (T **host_pointer, T **device_pointer, size_t size) into cuda_util.cu
...
/*void setup2(char *data_file, double& nx, double& ny, double& nz, double& omega, double& densities, double& population, double velocities)*/
void setup2(char *data_file, ptrdiff_t& nx, ptrdiff_t& ny, ptrdiff_t& nz, double& omega)
{
	// Set cuda device to use
	cudaSetDevice(0);
	cudaFuncSetCacheConfig(iterate_kernel, cudaFuncCachePreferL1);
